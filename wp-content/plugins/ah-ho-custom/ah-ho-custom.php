...
define('AH_HO_CUSTOM_VERSION', '1.7.4');
...
function ah_ho_custom_init() {
    ...
    // Include free-shipping links (signed, expiring private links that grant
    // free delivery below the $60 threshold) [FIX #4796]
    ...
    // Include guest account backfill (one-off admin tool for the guest orders
    // placed before guest-account-linking.php shipped)
    if (is_admin()) {
        require_once AH_HO_CUSTOM_PLUGIN_DIR . 'includes/guest-account-backfill.php';
    }
