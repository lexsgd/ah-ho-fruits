<?php
	 ...
	 * @subpackage  Core
	 * @author      FusionRedux Framework Team
	 */
// Exit if accessed directly
	...
	if ( ! class_exists( 'FusionReduxFrameworkInstances' ) ) {
		// Instance Container
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_instances.php' );
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/lib.fusionredux_instances.php' );
	}
...
// Don't duplicate me!
	if ( ! class_exists( 'FusionReduxFramework' ) ) {
		...
		// FusionRedux CDN class
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_cdn.php' );

		// FusionRedux API class  :)
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_api.php' );

		// General helper functions
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_helpers.php' );

		// General functions
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_functions.php' );
		// require_once dirname( __FILE__ ) . '/inc/class.p.php';

		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.thirdparty.fixes.php' );

		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_filesystem.php' );

		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/class.fusionredux_admin_notices.php' );

		// ThemeCheck checks
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/themecheck/class.fusionredux_themecheck.php' );

		// Welcome
		require_once wp_normalize_path( dirname( __FILE__ ) . '/inc/welcome/welcome.php' );

		/**
		 * Main FusionReduxFramework class
		 *
		 * @since       1.0.0
		 */
		class FusionReduxFramework {

			// ATTENTION DEVS
			// Please update the build number with each push, no matter how small.
			// This will make for easier support when we ask users what version they are using.

			public static $_version = '3.5.9.8';
			public static $_dir;
			public static $_url;
			...
			public static function init() {
				...
				self::$wp_content_url = trailingslashit( FusionRedux_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) ) );

				// See if FusionRedux is a plugin or not
			...
			public $framework_url = 'http://www.fusionreduxframework.com/';
			...
			public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
				...
				if ( isset ( $_POST ) && isset ( $_POST['action'] ) && $_POST['action'] == 'heartbeat' ) {
					...
					// Hook, for purists.
					if ( ! has_action( 'fusionredux/ajax/heartbeat' ) ) {
						do_action( 'fusionredux/ajax/heartbeat', $this );
					}
				...
				FusionRedux_Functions::$_parent     = $this;
				...
				/**
				 * filter 'fusionredux/args/{opt_name}'
				 *
				 * @param  array $args FusionReduxFramework configuration
				 */
				$this->args = apply_filters( "fusionredux/args/{$this->args['opt_name']}", $this->args );

				/**
				 * filter 'fusionredux/options/{opt_name}/args'
				 *
				 * @param  array $args FusionReduxFramework configuration
				 */
				$this->args = apply_filters( "fusionredux/options/{$this->args['opt_name']}/args", $this->args );
				...
				if ( ! empty ( $this->args['opt_name'] ) ) {
					...
					/**
					 * filter 'fusionredux-sections'
					 *
					 * @deprecated
					 *
					 * @param  array $sections field option sections
					 */
					$this->sections = apply_filters( 'fusionredux-sections', $sections ); // REMOVE LATER
					/**
					 * filter 'fusionredux-sections-{opt_name}'
					 *
					 * @deprecated
					 *
					 * @param  array $sections field option sections
					 */
					$this->sections = apply_filters( "fusionredux-sections-{$this->args['opt_name']}", $this->sections ); // REMOVE LATER
					/**
					 * filter 'fusionredux/options/{opt_name}/sections'
					 *
					 * @param  array $sections field option sections
					 */
					$this->sections = apply_filters( "fusionredux/options/{$this->args['opt_name']}/sections", $this->sections );
					...
					if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
						add_action( 'network_admin_edit_fusionredux_' . $this->args['opt_name'], array(
							$this,
							'save_network_page'
						), 10, 0 );
					...
					// 	new fusionreduxDashboardWidget($this);
					//
					// 	if ( ! isset ( $GLOBALS['fusionredux_notice_check'] ) ) {
					// 		require_once 'core/newsflash.php';
					//
					// 		$params = array(
					// 			'dir_name'    => 'notice',
					// 			'server_file' => 'http://fusionreduxframework.com/wp-content/uploads/fusionredux/fusionredux_notice.json',
					// 			'interval'    => 3,
					// 			'cookie_id'   => 'fusionredux_blast',
					// 		);
					//
					// 		new fusionreduxNewsflash( $this, $params );
					// 		$GLOBALS['fusionredux_notice_check'] = 1;
					// 	}
					// }
				}
...
// __construct()
			...
			private function set_fusionredux_content() {
				$upload_dir        = wp_upload_dir();
				self::$_upload_dir = $upload_dir['basedir'] . '/fusionredux/';
				self::$_upload_url = set_url_scheme( $upload_dir['baseurl'] . '/fusionredux/' );
			}
			...
			private function set_default_args() {
				$this->args = array(
					...
					'class'                     => '',
					// Class that gets appended to all fusionredux-containers
					'admin_bar'                 => true,
					...
					'dev_mode'                  => true,
					'templates_path'            => '',
					// Path to the templates file for various FusionRedux elements
					'ajax_save'                 => true,
			...
			public function network_admin_bar( $wp_admin_bar ) {
				...
				$args = array(
					...
					'meta'   => array( 'class' => 'fusionredux-network-admin' )
				);
			...
			public function _update_check() {
				...
				if ( ! isset ( $GLOBALS['fusionredux_update_check'] ) ) {
					FusionRedux_Functions::updateCheck( self::$_version );
					$GLOBALS['fusionredux_update_check'] = 1;
				}
			}

			public function _admin_notices() {
				FusionRedux_Admin_Notices::adminNotices( $this->admin_notices );
			}

			public function _dismiss_admin_notice() {
				FusionRedux_Admin_Notices::dismissAdminNotice();
			}
...
// set()
			...
			private function set_global_variable() {
				if ( $this->args['global_variable'] ) {
					...
					/**
					 * filter 'fusionredux/options/{opt_name}/global_variable'
					 *
					 * @param array $value option value to set global_variable with
					 */
					$GLOBALS[ $this->args['global_variable'] ] = apply_filters( "fusionredux/options/{$this->args['opt_name']}/global_variable", $this->options );
...
// set_global_variable()
			...
			public function set_options( $value = '' ) {
				...
				if ( ! empty ( $value ) ) {
					...
					//do_action( "fusionredux-saved-{$this->args['opt_name']}", $value ); // REMOVE
					//do_action( "fusionredux/options/{$this->args['opt_name']}/saved", $value, $this->transients['changed_values'] );
				}
			}
// set_options()
			...
			public function get_options() {
				...
				/**
				 * action 'fusionredux/options/{opt_name}/options'
				 *
				 * @param mixed $value option values
				 */
				$this->options = apply_filters( "fusionredux/options/{$this->args['opt_name']}/options", $this->options );
...
//return $data;
				/**
				 * filter 'fusionredux/options/{opt_name}/wordpress_data/{type}/'
				 *
				 * @deprecated
				 *
				 * @param string $data
				 */
				$data = apply_filters( "fusionredux/options/{$this->args['opt_name']}/wordpress_data/$type/", $data ); // REMOVE LATER

				/**
				 * filter 'fusionredux/options/{opt_name}/data/{type}'
				 *
				 * @param string $data
				 */
				$data = apply_filters( "fusionredux/options/{$this->args['opt_name']}/data/$type", $data );
				...
				if ( empty ( $data ) && ! empty ( $type ) ) {
					...
					if ( ! empty ( $type ) && empty ( $data ) ) {
						...
						if ( $type == "categories" || $type == "category" ) {
							...
							/**
							 * filter 'fusionredux-font-icons'
							 *
							 * @deprecated
							 *
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( 'fusionredux-font-icons', array() ); // REMOVE LATER
							 ...
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( 'fusionredux/font-icons', $font_icons );

							/**
							 * filter 'fusionredux/{opt_name}/field/font/icons'
							 *
							 * @deprecated
							 *
							 * @param array $font_icons array of elusive icon classes
							 */
							$font_icons = apply_filters( "fusionredux/{$this->args['opt_name']}/field/font/icons", $font_icons );
...
// show()
			...
			public function _default_values() {
						...
						if ( isset ( $section['fields'] ) ) {
							foreach ( $section['fields'] as $k => $field ) {
								...
								if ( $field['type'] == "section" && isset ( $field['indent'] ) && $field['indent'] == "true" ) {
									...
									$field['class'] .= " fusionredux-section-indent-start";
				...
				/**
				 * filter 'fusionredux/options/{opt_name}/defaults'
				 *
				...
				$this->options_defaults             = apply_filters( "fusionredux/options/{$this->args['opt_name']}/defaults", $this->options_defaults, $this->transients['changed_values'] );
			...
			private function _default_cleanup() {
				...
				if ( FusionRedux_Helpers::isLocalHost() || ( FusionRedux_Helpers::isWpDebug() ) ) {
			...
			public function _admin_bar_add_menu() {
				...
				if ( $menu ) {
					...
					if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
						foreach ( $submenu[ $this->args["page_slug"] ] as $index => $fusionredux_options_submenu ) {
							$subnodeargs = array(
								'id'     => $this->args["page_slug"] . '_' . $index,
								'title'  => $fusionredux_options_submenu[0],
								'parent' => $this->args["page_slug"],
								'href'   => admin_url( 'admin.php?page=' . $fusionredux_options_submenu[2] ),
							);
					...
					$nodeargs = array(
						...
						'title' => $title,
						// $theme_data->get( 'Name' ) . " " . __( 'Options', 'fusionredux-framework-demo' ),
...
// _admin_bar_add_menu()
			...
			public function _enqueue() {
				require_once wp_normalize_path( dirname( __FILE__ ) . '/core/enqueue.php');
				$enqueue = new fusionreduxCoreEnqueue ( $this );
				$enqueue->init();
			}
// _enqueue()
			...
			public function _load_page() {
				...
				if ( $this->args['help_sidebar'] != '' ) {
					...
					// about hints.
					if ( true == $this->show_hints ) {
						$screen->set_help_sidebar( '<p><strong>FusionRedux Framework</strong><br/><br/>Hint Tooltip Preferences</p>' );
					}
				}

				/**
				 * action 'fusionredux-load-page-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param object $screen WP_Screen
				 */
				do_action( "fusionredux-load-page-{$this->args['opt_name']}", $screen ); // REMOVE

				/**
				 * action 'fusionredux/page/{opt_name}/load'
				 *
				 * @param object $screen WP_Screen
				 */
				do_action( "fusionredux/page/{$this->args['opt_name']}/load", $screen );
			}
// _load_page()

			/**
			 * Do action fusionredux-admin-head for options page
			 *
			 * @since       1.0.0
			 * @access      public
			 * @return      void
			...
			public function admin_head() {
				/**
				 * action 'fusionredux-admin-head-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param object $object FusionReduxFramework
...
// get_default_output_string()
			...
			public function get_header_html( $field ) {
				...
				if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
					...
					if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
						...
						$hint = apply_filters( 'fusionredux/hints/html', $hint, $field, $this->args );
						...
						if ( 'true' == $metaVal || empty ( $metaVal ) && empty( $hint )  ) {
							...
							$hint = '<div class="fusionredux-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' . $titleParam . '" qtip-content="' . $contentParam . '">&nbsp;<i class="' . ( isset( $this->args['hints']['icon'] ) ? $this->args['hints']['icon'] : '' ) . '"></i></div>';
						}
					}
				...
				if ( ! empty ( $th ) ) {
					$th = '<div class="fusionredux_field_th">' . $th . '</div>';
				}
			...
			public function _register_settings() {
				...
				foreach ( $this->sections as $k => $section ) {
					...
					/**
					 * filter 'fusionredux-section-{index}-modifier-{opt_name}'
					 *
					 * @param array $section section configuration
					 */
					$section = apply_filters( "fusionredux-section-{$k}-modifier-{$this->args['opt_name']}", $section );

					/**
					 * filter 'fusionredux/options/{opt_name}/section/{section.id}'
					 *
					 * @param array $section section configuration
					 */
					if ( isset ( $section['id'] ) ) {
						$section = apply_filters( "fusionredux/options/{$this->args['opt_name']}/section/{$section['id']}", $section );
					}
					...
					if ( isset ( $section['fields'] ) ) {
						foreach ( $section['fields'] as $fieldk => $field ) {
							...
							/**
							 * filter 'fusionredux/options/{opt_name}/field/{field.id}'
							 *
							 * @param array $field field config
							 */
							$field = apply_filters( "fusionredux/options/{$this->args['opt_name']}/field/{$field['id']}/register", $field );
							...
							/**
							 * filter 'fusionredux-field-{field.id}modifier-{opt_name}'
							 *
							 * @deprecated
							 *
							 * @param array $field field config
							 */
							$field = apply_filters( "fusionredux-field-{$field['id']}modifier-{$this->args['opt_name']}", $field ); // REMOVE LATER
							 ...
							 * @param array $field field config
							 */
							$field = apply_filters( "fusionredux/options/{$this->args['opt_name']}/field/{$field['id']}", $field );
							...
							/**
							 * action 'fusionredux/options/{opt_name}/field/field.type}/register'
							 */
							do_action( "fusionredux/options/{$this->args['opt_name']}/field/{$field['type']}/register", $field );
							...
							if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
								...
								if ( isset ( $field['hidden'] ) && $field['hidden'] ) {
									$field['label_for'] = 'fusionredux_hide_field';
								}
								if ( $this->args['options_api'] == true ) {
									add_settings_field(
				...
				/**
				 * action 'fusionredux-register-settings-{opt_name}'
				 *
				 * @deprecated
				 */
				do_action( "fusionredux-register-settings-{$this->args['opt_name']}" ); // REMOVE
...
// _register_settings()
			...
			private function _register_extensions() {
				...
				if ( isset( $this->old_opt_name ) ) {
					do_action( "fusionredux/extensions/{$this->old_opt_name}/before", $this );
				}
				...
				foreach ( $folders as $folder ) {
					...
					$extension_class = 'FusionReduxFramework_Extension_' . $folder;

					/**
					 * filter 'fusionredux-extensionclass-load'
					 *
					 * @deprecated
					 *
				...
				/**
				 * action 'fusionredux-register-extensions-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param object $object FusionReduxFramework
				...
				if ( isset( $this->old_opt_name ) && ! empty( $this->old_opt_name ) ) {
					do_action( "fusionredux/extensions/{$this->old_opt_name}", $this );
				}
			}
			...
			public function _validate_options( $plugin_options ) {
				...
				if ( isset ( $plugin_options['fusionredux-no_panel'] ) ) {
					$keys = explode( '|', $plugin_options['fusionredux-no_panel'] );
					foreach ( $keys as $key ) {
						$plugin_options[ $key ] = $this->options[ $key ];
					}
					if ( isset ( $plugin_options['fusionredux-no_panel'] ) ) {
						unset ( $plugin_options['fusionredux-no_panel'] );
					}
				}
				...
				if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
					...
					if ( ! empty ( $imported_options ) && is_array( $imported_options ) && ( ( isset ( $imported_options['fusionredux-backup'] ) && $imported_options['fusionredux-backup'] == '1' ) || ( isset ( $imported_options['avadaredux-backup'] ) && $imported_options['avadaredux-backup'] == '1' ) ) ) {
						 ...
						 */
						do_action_ref_array( "fusionredux/options/{$this->args['opt_name']}/import", array(
							&$plugin_options,
							$imported_options,
							$this->transients['changed_values']
						) );

						setcookie( 'fusionredux_current_tab', '', 1, '/', $time + 1000, "/" );
				...
				if ( ! empty ( $plugin_options['defaults-section'] ) ) {
					...
					//setcookie("fusionredux-saved-{$this->args['opt_name']}", 'defaults_section', time() + 1000, "/");
					unset ( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['fusionredux-section'] );
...
//                }
				...
				/**
				 * action 'fusionredux-validate-{opt_name}'
				 *
				 * @deprecated
				 *
				 * @param  &array [&$plugin_options, fusionredux_options]
				 */
				do_action_ref_array( "fusionredux-validate-{$this->args['opt_name']}", array(
					&$plugin_options,
					$this->options
				) ); // REMOVE
				...
				/**
				 * action 'fusionredux/options/{opt_name}/validate'
				 *
				 * @param  &array [&$plugin_options, fusionredux_options]
				 */
				do_action_ref_array( "fusionredux/options/{$this->args['opt_name']}/validate", array(
					&$plugin_options,
					$this->options,
					$this->transients['changed_values']
				) );
			...
			public function ajax_save() {
				...
				$fusionredux = FusionReduxFrameworkInstances::get_instance( $_POST['opt_name'] );
					...
					if ( ! empty ( $values ) ) {
						...
						try {
							...
							if ( ! empty ( $values['defaults'] ) || ! empty ( $values['defaults-section'] ) ) {
								...
								unset( $backup_values['defaults'] );
								unset( $backup_values['compiler'] );
								unset( $backup_values['fusionredux-section'] );
								unset( $backup_values['import_code'] );
								unset( $backup_values['import_link'] );

								$backup_values['fusionredux-backup'] = '1';
								$content     = json_encode( $backup_values );
							...
							$return_array = array(
								...
								'errors'   => isset ( $fusionredux->localize_data['errors'] ) ? $fusionredux->localize_data['errors'] : null,
								'warnings' => isset ( $fusionredux->localize_data['warnings'] ) ? $fusionredux->localize_data['warnings'] : null,
							);
				...
				if ( isset( $return_array ) ) {
					if ( $return_array['status'] == "success" ) {
						...
						$panel = new fusionreduxCorePanel ( $fusionredux );
						ob_start();
					...
					echo json_encode( apply_filters( "fusionredux/options/{$this->args['opt_name']}/ajax_save/response", $return_array ) );
				}
				...
				foreach ( $sections as $k => $section ) {
					if ( isset ( $section['fields'] ) ) {
						foreach ( $section['fields'] as $fkey => $field ) {
							...
							if ( isset ( $field['validate'] ) ) {
								$validate = 'FusionRedux_Validation_' . $field['validate'];

								if ( ! class_exists( $validate ) ) {
									/**
									 * filter 'fusionredux-validateclass-load'
									 *
									 * @deprecated
									 *
...
// section_menu()
			...
			public function generate_panel() {
				require_once wp_normalize_path( dirname( __FILE__ ) . '/core/panel.php' );
				$panel = new fusionreduxCorePanel ( $this );
				$panel->init();
				$this->set_transients();
			}
			...
			public function _section_desc( $section ) {
				...
				if ( isset ( $this->sections[ $id ]['desc'] ) && ! empty ( $this->sections[ $id ]['desc'] ) ) {
					echo '<div class="fusionredux-section-desc">' . $this->sections[ $id ]['desc'] . '</div>';
				}
				if ( isset ( $this->sections[ $id ]['highlight'] ) && ! empty ( $this->sections[ $id ]['highlight'] ) ) {
					echo '<div class="fusionredux-element-highlight">' . $this->sections[ $id ]['highlight'] . '</div>';
				}
			}
			...
			public function _field_input( $field, $v = null ) {
				...
				if ( isset ( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
					...
					/**
					 * action 'fusionredux-before-field-{opt_name}'
					 *
					 * @deprecated
					 *
					 ...
					 */
					do_action( "fusionredux-before-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

					/**
					 * action 'fusionredux/field/{opt_name}/{field.type}/callback/before'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/{$field['type']}/callback/before", array(
						&$field,
						&$value
					) );

					/**
					 * action 'fusionredux/field/{opt_name}/callback/before'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/callback/before", array(
						&$field,
						&$value
					) );
					...
					/**
					 * action 'fusionredux-after-field-{opt_name}'
					 *
					 * @deprecated
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action( "fusionredux-after-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

					/**
					 * action 'fusionredux/field/{opt_name}/{field.type}/callback/after'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/{$field['type']}/callback/after", array(
						&$field,
						&$value
					) );

					/**
					 * action 'fusionredux/field/{opt_name}/callback/after'
					 *
					 * @param array  $field field data
					 * @param string $value field.id
					 */
					do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/callback/after", array(
						&$field,
						&$value
					) );
...
//                    $class_file = apply_filters( 'fusionredux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
						/**
					...
					if ( class_exists( $field_class ) ) {
						...
						/**
						 * action 'fusionredux/field/{opt_name}/{field.type}/render/before'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/{$field['type']}/render/before", array(
							&$field,
							&$value
						) );

						/**
						 * action 'fusionredux/field/{$this->args['opt_name']}/render/before'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/render/before", array(
							&$field,
							&$value
						) );
					 ...
					 */
						...
						/**
						 * filter 'fusionredux-field-{opt_name}'
						 *
						 * @deprecated
						 *
						 * @param string $file_path validation class file path
						...
						/**
						 * action 'fusionredux/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );

						/**
						 * action 'fusionredux/field/{opt_name}/fieldset/before/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );
						...
						if ( isset( $field['full_width'] ) && $field['full_width'] == true ) {
							$class_string .= "fusionredux_remove_th";
						}
						...
						echo '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="fusion_theme_options-' . $field['id'] . ' ' . $hidden . 'fusionredux-field-container fusionredux-field fusionredux-field-init fusionredux-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
						//}
						...
						/**
						 * action 'fusionredux/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );

						/**
						 * action 'fusionredux/field/{opt_name}/fieldset/after/{opt_name}'
						 *
						 * @param array  $field field data
						 * @param string $value field id
						 */
						do_action_ref_array( "fusionredux/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}", array(
							&$field,
							&$value
						) );
					}
...
//                        }
			 ...
			 * @return  array $result
			 */
			function fusionredux_parse_str( $string ) {
				if ( '' == $string ) {
					return false;
				}
				...
				foreach ( $pairs as $key => $pair ) {
					...
					if ( ! isset( $result[ $k ] ) ) {
						...
						$result[ $k ] = $this->fusionredux_array_merge_recursive_distinct( $result[ $k ], $params[ $k ] );
					}
				}
			 ...
			 * @return  array $merged
			 */
			function fusionredux_array_merge_recursive_distinct( array $array1, array $array2 ) {
				$merged = $array1;

				foreach ( $array2 as $key => $value ) {
					if ( is_array( $value ) && isset( $merged[$key] ) && is_array( $merged[$key] ) ) {
						$merged[$key] = $this->fusionredux_array_merge_recursive_distinct( $merged[$key], $value );
