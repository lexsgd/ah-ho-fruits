...
class PyreThemeFrameworkMetaboxes {
	...
	public function __construct() {
		...
		add_filter( 'awb_states_params', [ $this, 'add_states_params' ], 10, 2 );
	}
	...
	public function render_tab_fields( $tab_data, $repeater = false ) {
		...
		foreach ( $tab_data['fields'] as $field ) {
			...
			$field['states']               = isset( $field['states'] ) ? $field['states'] : [];
			$field['default_state_option'] = isset( $field['default_state_option'] ) ? $field['default_state_option'] : '';
			...
			switch ( $field['type'] ) {
				...
				case 'checkbox-buttonset':
					...
					break;
				case 'color-alpha':
					$this->color( $field['id'], $field['label'], $field['description'], true, $field['dependency'], $field['default'], $field['responsive'], $field['allow_globals'], $field['state'], $field['states'], $field['default_state_option'] );
					break;
				case 'color':
					$this->color( $field['id'], $field['label'], $field['description'], false, $field['dependency'], $field['default'], $field['responsive'], $field['allow_globals'], $field['state'], $field['states'], $field['default_state_option'] );
					break;
				case 'typography':
					$this->typography( $field['id'], $field['label'], $field['description'], $field['dependency'], $field );
					break;
				case 'media':
				case 'media_url':
				case 'upload':
				...
				case 'multiple_select':
					...
					break;
				case 'select':
					$this->select( $field['id'], $field['label'], $field['choices'], $field['description'], $field['default'], $field['dependency'], $repeater, $field['responsive'], $field['state'], $field['states'], $field['default_state_option'] );
					break;
				case 'dimensions':
				...
				case 'hubspot_map':
					...
					break;
				case 'hubspot_consent_map':
					$this->hubspot_consent_map( $field['id'], $field['label'], $field['choices'], $field['description'], $field['dependency'], $field['default'], $field['responsive'] );
					break;
				case 'mailchimp_map':
				...
				<div class="awb-typography">
					...
					<?php if ( isset( $field['choices']['font-family'] ) && false !== $field['choices']['font-family'] ) : ?>
						<?php
							...
							$family_default      = ( isset( $field['default'], $field['default']['font-family'] ) ? $field['default']['font-family'] : '' );
							$family_value        = ( isset( $value['font-family'] ) ? $value['font-family'] : $family_default );
							$display_backup_font = ( isset( $field['backup_font'] ) ? $field['backup_font'] : true );
							...
							<div class="fusion-skip-init fusion-select-field<?php echo ( is_rtl() ) ? ' fusion-select-field-rtl' : ''; ?>">
								<div class="fusion-select-preview-wrap">
									<span class="fusion-select-preview">
										<?php if ( '' !== $family_value ) : ?>
											<!-- Todo: Maybe add font name via JS. -->
											<?php echo esc_html( $family_value ); ?>
										<?php else : ?>
								...
								<input type="hidden" id="<?php echo esc_attr( $family_id ); ?>" name="<?php echo esc_attr( $this->format_option_name( $family_id ) ); ?>" value="<?php echo esc_attr( $family_value ); ?>" data-default="<?php echo esc_attr( $family_default ); ?>" class="input-font_family fusion-select-option-value awb-typo-input" data-subset="font-family">
							</div>
							<span class="awb-global-label"></span>
						</div>

						<?php if ( $display_backup_font ) : ?>
						<div class="input-wrapper font-backup fusion-font-backup-wrapper">
							...
							<div class="fusion-skip-init fusion-select-field<?php echo ( is_rtl() ) ? ' fusion-select-field-rtl' : ''; ?>">
								<div class="fusion-select-preview-wrap">
									<span class="fusion-select-preview">
										<?php if ( isset( $value['font-backup'] ) && '' !== $value['font-backup'] ) : ?>
											<?php echo esc_html( $value['font-backup'] ); ?>
										<?php else : ?>
								...
								<input type="hidden" id="fusion-typography-font-backup-<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $this->format_option_name( $id . '[font-backup]' ) ); ?>" value="<?php echo esc_attr( $value['font-backup'] ); ?>" class="fusion-select-option-value">
							</div>
						</div>
						<?php endif; ?>
						...
						<?php
							...
							$variant_default = ( isset( $field['default'], $field['default']['variant'] ) ? $field['default']['variant'] : '' );
							$variant_value   = ( isset( $value['variant'] ) ? $value['variant'] : $family_default );
						...
						<div class="input-wrapper fusion-builder-typography">
							...
							<div class="input fusion-typography-select-wrapper">
								<select name="<?php echo esc_attr( $this->format_option_name( $variant_id ) ); ?>" class="input-variant variant" id="<?php echo esc_attr( $variant_id ); ?>" data-default="<?php echo esc_attr( $variant_default ); ?>" data-value="<?php echo esc_attr( $variant_value ); ?>" data-subset="variant"></select>
								<div class="fusiona-arrow-down"></div>
							</div>
						</div>

						<?php foreach ( [ 'font-size', 'line-height', 'letter-spacing' ] as $setting_prop ) : ?>
							<?php if ( $field['choices'] ) : ?>
								<?php
								$string_map = [
									'font-size'      => __( 'Font Size', 'Avada' ),
									'line-height'    => __( 'Line Height', 'Avada' ),
									'letter-spacing' => __( 'Letter Spacing', 'Avada' ),
								];

								$field_id      = $id . '[' . $setting_prop . ']';
								$field_default = ( isset( $field['default'], $field['default'][ $setting_prop ] ) ? $field['default'][ $setting_prop ] : '' );
								$field_value   = ( isset( $value[ $setting_prop ] ) ? $value[ $setting_prop ] : $field_default );
								?>
								<div class="input-wrapper awb-contains-global third">
									<div class="awb-typo-heading">
										<label><?php echo esc_attr( $string_map[ $setting_prop ] ); ?></label>
										<?php if ( $allow_global_button ) { ?>
											<span class="awb-global"><i class="fusiona-globe" aria-hidden="true"></i></span>
										<?php } ?>
									</div>
									<div class="input">
										<input type="text" data-subset="<?php echo esc_attr( $setting_prop ); ?>" class="awb-typo-input" name="<?php echo esc_attr( $this->format_option_name( $field_id ) ); ?>" value="<?php echo esc_attr( $field_value ); ?>">
										<span class="awb-global-label"></span>
									</div>
								</div>
							<?php endif; ?>
						...
						<?php if ( isset( $field['choices'], $field['choices']['text-transform'] ) && false !== $field['choices']['text-transform'] ) : ?>
							<?php
