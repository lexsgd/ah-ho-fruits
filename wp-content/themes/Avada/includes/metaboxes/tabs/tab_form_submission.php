...
function avada_page_options_tab_form_submission( $sections ) {
	...
	if ( class_exists( 'FusionBuilder' ) ) {
		...
		if ( ! empty( $custom_actions ) ) {
			foreach ( $custom_actions as $key => $custom_action ) {
				$choices[ $key ] = $custom_action['label'];
			}
		}
	}
	$sections['form_submission'] = [
		...
		'fields'   => [
			...
			'post_method_note' => [
				'type'        => 'custom',
				'description' => '<div class="fusion-redux-important-notice">' . __( 'Notifications will be only sent when submission type is set to "AJAX".', 'Avada' ) . '</div>',
				'id'          => 'post_method_note',
				'transport'   => 'postMessage',
				'dependency'  => [
					[
