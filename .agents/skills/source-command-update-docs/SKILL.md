...
Migrated from source command `update-docs` into a Codex skill. Invoke it as `$source-command-update-docs` and manually rewrite any slash-command behavior that depended on provider-specific runtime expansion.
