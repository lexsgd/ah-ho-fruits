...
def persist_refresh(new_refresh):
    ...
    with open(PROJ_ENV) as f:
        for line in f:
            if line.startswith("QBO_B2C_REFRESH_TOKEN="):
                lines.append(f"QBO_B2C_REFRESH_TOKEN={new_refresh}\n")
                found = True
            else:
                lines.append(line)
    if found:
...
def main():
    ...
    client_id = proj.get("QBO_B2C_CLIENT_ID")
    client_secret = proj.get("QBO_B2C_CLIENT_SECRET")
    realm = proj.get("QBO_B2C_REALM_ID")
    refresh = proj.get("QBO_B2C_REFRESH_TOKEN")
    if not all([client_id, client_secret, realm, refresh]):
        print("ERROR: missing QBO_B2C_CLIENT_ID/SECRET/REALM_ID/REFRESH_TOKEN (project .env)")
        sys.exit(1)
